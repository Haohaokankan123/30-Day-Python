...
function buildSidebar() {
  ...
  DAYS.forEach(d => {
    ...
    item.className = 'day-nav-item' + (done ? ' done' : '') + (active ? ' active' : '');
    ...
    `;
    item.onclick = () => { loadDay(d.day); closeSidebarOnMobile(); };
...
function loadDay(n) {
  ...
  setTimeout(() => { if (window.Prism) Prism.highlightAll(); }, 60);
...
function buildDayDots(current) {
  ...
  for (let i = start; i <= end; i++) {
    ...
    dot.className = 'day-dot' + (i === current ? ' active' : '') + (state.completed.has(i) ? ' done' : '');
...
function renderExamples(examples) {
  if (!examples || !examples.length) return '<p style="color:var(--text-3);padding:40px">No examples for this day.</p>';
...
function renderExercisesTab(day) {
  ...
  const quizHtml = day.quiz && day.quiz.length ? renderQuiz(day.quiz, day.day) : '';
...
function switchTab(btn, panelId) {
  ...
  if (panel) {
    ...
    if (panelId === 'examples' && window.Prism) setTimeout(() => Prism.highlightAll(), 60);
  }
...
function showCelebration(n) {
  ...
  document.getElementById('celebrationMsg').textContent = `Day ${n} complete! ${msgs[n % msgs.length]}`;
...
function filterDays(q) {
  ...
  document.querySelectorAll('.day-nav-item').forEach(item => {
    const title = item.querySelector('.day-nav-title').textContent.toLowerCase();
    const num = item.querySelector('.day-nav-num').textContent;
    item.style.display = (!query || title.includes(query) || num.includes(query)) ? '' : 'none';
  });
}
